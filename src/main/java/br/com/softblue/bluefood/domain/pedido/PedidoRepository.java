...
public interface PedidoRepository extends JpaRepository<Pedido, Integer>{
	...
	public List<Pedido> findByIdAndRestaurante_Id(Integer pedidoId, Integer restauranteId);
	...
	public List<Object[]> findItensPorFaturamento(Integer restauranteId, Integer itemCardapioId, LocalDateTime dataInicial, LocalDateTime dataFinal);
	
	@Query("SELECT i.itemCardapio.nome, SUM(i.quantidade), SUM(i.preco * i.quantidade) FROM Pedido p INNER JOIN p.itensPedido i "
			+ "WHERE p.restaurante.id = ?1 AND p.data BETWEEN ?2 AND ?3 GROUP BY i.itemCardapio.nome")
	public List<Object[]> findItensPorFaturamento(Integer restauranteId, LocalDateTime dataInicial, LocalDateTime dataFinal);
}
